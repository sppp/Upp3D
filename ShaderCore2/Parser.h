...
struct Parser {
    Parser(String source, String file_name);
...
protected:
    ...
    CHECK_RETURN bool ParseTopLevel(Array<TopLevel> &top);
    ...
    CHECK_RETURN bool IsConstantValue(Expression *expression) const;
    CHECK_RETURN bool IsConstant(Expression *expression) const;
    CHECK_RETURN bool IsConstant(One<Expression>& expression) const {return IsConstant(&*expression);}

    void Fatal(String msg);

    CHECK_RETURN ConstantExpression *Evaluate(Expression *expression);
    CHECK_RETURN ConstantExpression *Evaluate(One<Expression> &expression) {return Evaluate(&*expression);}
    ...
    Type *FindType(String identifier);
    Variable *FindVariable(String identifier);
...
private:
    ...
    Array<Memory> memory; // Memory of AST held here
    Vector<String> strings; // Memory of strings held here
};
