...
#define Print(...) \
    do { \
        UPP::StdLog() << Format(__VA_ARGS__); \
    } while (0)

static void PrintExpression(const Expression& expression);
static void PrintExpression(const One<Expression>& expression) {PrintExpression(*expression);}
static void PrintStatement(const Statement& statement);
static void PrintStatement(const One<Statement>& statement) {PrintStatement(*statement);}

static void PrintBuiltin(const Builtin& builtin) {
    Print("%s", kTypes[builtin.type]);
}

static void PrintType(const Type& type) {
    if (type.builtin)
        PrintBuiltin((Builtin&)type);
    else
        Print("%s", ((Struct&)type).name);
}
static void PrintType(const One<Type>& type) {PrintType(*type);}
...
static void PrintUIntConstant(UIntConstant& expression) {
    Print("%d", (int64)expression.value);
}

static void PrintFloatConstant(FloatConstant& expression) {
    Print("%1!n", expression.value);
}
...
static void PrintArraySize(const Array<ConstantExpression> &array_sizes) {
...
static void PrintVariable(Variable& variable, bool name_only = false) {
...
static void PrintGlobalVariable(GlobalVariable& variable) {
    Array<LayoutQualifier> &qualifiers = variable.layout_qualifiers;
...
static void PrintIfStetement(IfStatement& statement) {
    ...
    Print(")");
    PrintStatement(*statement.then_stmt);
    if (statement.else_stmt) {
        Print("else");
        if (statement.else_stmt->type == Statement::STMT_IF)
            Print(" ");
        PrintStatement(*statement.else_stmt);
    }
}
...
int Main(int argc, char **argv) {
    ...
    for (int i = 0; i < sources.GetCount(); i++) {
        ...
        One<TU> tu = p.Parse(sources[i].shader_type);
        if (tu) {
            PrintTU(*tu);
        } else {
            Cerr() << p.GetError() << "\n";
