...
Parser::Parser(String source, String file_name)
    : ast(0)
    , lexer(source)
    , file_name(file_name)
{
...
bool Parser::IsConstantValue(Expression *expression) const {
...
void Parser::Fatal(String msg) {
    error = Format("%s:%d:%d: error: %s", file_name, lexer.GetLine(), lexer.GetColumn(), msg);
    strings.Add(error);
}
...
CHECK_RETURN TU *Parser::Parse(int type) {
    ...
    for (;;) {
        ...
        if (lexer.GetError().GetCount()) {
            Fatal(lexer.GetError());
            return 0;
        }
        ...
        if (IsType(TYPE_semicolon)) {
            for (int i = 0; i < items.GetCount(); i++) {
                ...
                if (parse.initial_value) {
                    global->initial_value = Evaluate(parse.initial_value);
                    if (global->initial_value.IsEmpty())
                        return 0;
                }
                global->is_array = parse.is_array;
                global->array_sizes <<= parse.array_sizes;
                GlobalVariable* ptr = global.Detach();
                ast->globals.Add(ptr);
                scopes.Top().Add(ptr);
            }
            ...
            Fatal(Format("syntax error at top level %d", token.as_keyword));
            return 0;
        }
    }
    return ast.Detach();
...
CHECK_RETURN bool Parser::ParseLayout(TopLevel &current) {
    Array<LayoutQualifier> &qualifiers = current.layout_qualifiers;
    if (IsKeyword(KEYWORD_layout)) {
        ...
        while (!IsOperator(OPERATOR_paranthesis_end)) {
            ...
            int found = -1;
            qualifier->name = IsType(TYPE_identifier) ? token.as_identifier : "shared";
            ...
            if (found == -1) {
                Fatal(Format("unknown layout qualifier `%s'", qualifier->name));
                return false;
            }
            ...
            if (IsOperator(OPERATOR_assign)) {
                if (!kLayoutQualifiers[found].is_assign) {
                    Fatal(Format("unexpected layout qualifier value on `%s' layout qualifier", qualifier->name));
                    return false;
                }
                if (!Next()) // skip '='
                    return false;
                
                qualifier->initial_value = ParseExpression(END_CONDITION_COMMA | END_CONDITION_PARENTHESIS);
                if (qualifier->initial_value.IsEmpty())
                    return false;
                if (!IsConstant(qualifier->initial_value)) {
                    // TODO: check integer-constant-expression
                    Fatal(Format("value for layout qualifier `%s' is not a valid constant expression",
                        qualifier->name));
                    return false;
                }
                qualifier->initial_value = Evaluate(qualifier->initial_value);
                if (qualifier->initial_value.IsEmpty())
                    return false;
            }
            else if (kLayoutQualifiers[found].is_assign) {
                Fatal(Format("expected layout qualifier value for `%s' layout qualifier", qualifier->name));
                return false;
            }
...
CHECK_RETURN bool Parser::ParseTopLevel(Array<TopLevel> &items) {
    TopLevel item;
...
CHECK_RETURN Expression *Parser::ParseBinary(int lhsPrecedence, Expression *lhs, EndCondition end) {
    ...
    while (!IsEndCondition(end)) {
        int binary_precedence = token.GetPrecedence();
        if (binary_precedence < lhsPrecedence)
            break;
        ...
        // climb
        if (binary_precedence < rhsPrecedence) {
            if (!(rhs = ParseBinary(binary_precedence + 1, rhs, end)))
                return 0;
        }
...
CHECK_RETURN IfStatement *Parser::ParseIfStatement() {
    ...
    if (IS_KEYWORD(peek, KEYWORD_else)) {
        ...
        if (!Next()) // skip 'else'
            return 0;
        if (!(statement->else_stmt = ParseStatement()))
            return 0;
    }
    return statement;
}
...
CHECK_RETURN SwitchStatement *Parser::ParseSwitchStatement() {
    ...
    Vector<int> seen_ints;
    Vector<unsigned int> seen_uints;
        ...
        if (nextStatement->type == Statement::STMT_CASE_LABEL) {
            ...
            if (!caseLabel->is_default) {
                ...
                if (value->type == Expression::INT_CONST) {
                    ...
                    if (Shader::Find(seen_ints.begin(), seen_ints.end(), val) != seen_ints.end()) {
                        Fatal(Format("duplicate case label `%d'", val));
                        return 0;
                    }
                    seen_ints.Add(val);
                } else if (value->type == Expression::UINT_CONST) {
                    const unsigned int val = UVAL(value);
                    if (Shader::Find(seen_uints.begin(), seen_uints.end(), val) != seen_uints.end()) {
                        Fatal(Format("duplicate case label `%u'", val));
                        return 0;
                    }
                    seen_uints.Add(val);
                } else {
...
CHECK_RETURN DeclarationStatement *Parser::ParseDeclarationStatement(EndCondition condition) {
    ...
    for (;;) {
        ...
        String name = NewString(token.as_identifier);
        if (!Next()) // skip identifier
            return 0;
...
Type *Parser::FindType(String name) {
...
Variable *Parser::FindVariable(String identifier) {
