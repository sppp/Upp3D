...
void PrepareRequest(HttpRequest& h) {
	...
	h.UserAgent("Mozilla/5.0 (X11; Linux amd64; rv:78.0) Gecko/20100101 Firefox/78.0");
}
	...
	for(int i = 0; i < json_cats.GetCount(); i++) {
		...
		for(int i = 0; i < rendpass.GetCount(); i++) {
			...
			
			{
				...
				String channel = pass.GetAdd("channel");
				
				fout << pre << "name = " << name << "\n";
				fout << pre << "description = " << description << "\n";
				fout << pre << "type = " << type << "\n";
